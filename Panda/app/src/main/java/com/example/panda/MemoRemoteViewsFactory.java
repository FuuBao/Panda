...
class MemoRemoteViewsFactory implements RemoteViewsService.RemoteViewsFactory {
    ...
    public MemoRemoteViewsFactory(Context context) {
    ...
    public RemoteViews getViewAt(int position) {
        ...
        listviewWidget.setTextViewText(R.id.text, arrayList.get(position).content);

        // 항목 선택 이벤트 발생 시 인텐트에 담겨야 할 항목 데이터를 추가해주는 코드
        Intent dataIntent = new Intent();
        dataIntent.putExtra("item_title", arrayList.get(position).title);
